...
class ExpRefcatDownload(ExpTask):
    ...
    url = luigi.Parameter(
        default="https://archive.org/download/refcat_2022-01-03/refcat-doi-table-2022-01-03.json.zst"
    )
    ...
    def output(self):
        return luigi.LocalTarget(
            path=self.path(ext="tsv.zst", digest=True), format=Zstd
        )
...
class ExpStatsReportData(ExpTask):
    ...
    def requires(self):
        return {
            ...
            "common_institution": ExpStatsCommonDOIForInstitution(
                date=self.date, institution=self.institution, exp=self.exp
            ),
        ...
        data = {
            ...
            "institution": {
                self.institution: {
                    ...
                    "ratio_common_exp": (
                        sum(1 for _ in si.get("common_institution").open())
                        / sum(1 for _ in si.get("exp_unique").open())
                    ),
                },
            },
            "index": {
            ...
            "exp": {
                ...
                "stats_inbound": json.load(si.get("exp_inbound").open()).get(
                    "inbound_edges"
                ),
                "stats_outbound": json.load(si.get("exp_outbound").open()).get(
                    "outbound_edges"
                ),
            },
        }
